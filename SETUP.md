...
- **Bills (fb) / Sold** — RuStore `MAIN`+`ACTIVE`. В daily вешаются на **день когорты**: yearly = дата оплаты − 7д (старт триала), monthly = день оплаты. Поэтому у свежих триалов sold/bills появляются с задержкой ~7 дней. Возвраты (`MAIN CLOSED`) не считаются.
