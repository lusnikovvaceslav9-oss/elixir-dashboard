...
- **Trials** — AppMetrica `trial_started` (уники по дням); RuStore — сверка в meta
- **Bills / Sold** — RuStore MAIN ACTIVE; в daily на **день старта триала** (yearly: оплата − 7д), monthly — день оплаты. `payments_by_plan` в meta — разбивка yearly/monthly. CSV `rustore-payments.csv` — fallback
